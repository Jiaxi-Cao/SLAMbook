...
/// 位姿顶点
class VertexPose : public g2o::BaseVertex<6, Sophus::SE3d> { //变量维度，变量类型
   public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;

    virtual void setToOriginImpl() override { _estimate = Sophus::SE3d(); } //重写设置原点的函数

    /// left multiplication on Sophus::SE3d 重写更新优化变量的函数
    virtual void oplusImpl(const double *update) override {
        Vec6 update_eigen;
        update_eigen << update[0], update[1], update[2], update[3], update[4],
            update[5]; //这些update是什么类型
        _estimate = Sophus::SE3d::exp(update_eigen) * _estimate; //重写的李代数左乘更新
    }
